...
- **Rejected:** `bot.onTyping(handler)` method — should use existing `bot.on('typing', handler)` for consistency.
...
- **Key Patterns:**
  - Typing activity type `'typing'` is part of the `ActivityType` string literal union
